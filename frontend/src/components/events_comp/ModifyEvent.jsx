...
import { amptyEvent, compareObjects } from "../../constants";
import { useType, useShowModal } from "../../hooks";

const ModifyEvent = ({ oldData }) => {
  if (oldData === undefined || Object.keys(oldData).length === 0)
    oldData = amptyEvent;
  ...
  useEffect(() => {
    ...
    const isValid =
      ...
      !compareObjects(oldData, eventData);
    if (isValid !== ableToSubmit) setAbleToSubmit(isValid);
  }, [eventData]);

  return (
    <div className="sm:w-[60%] w-[90%] h-fit  bg-white rounded-xl gap-5 flex flex-col shadow-lg p-3 sm:p-5 items-center">
